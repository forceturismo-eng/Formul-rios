...
export function PaginaEquipe() {
  const { data, isLoading } = useQuery({
    queryKey: ['membros'],
    queryFn: () =>
      api<{ members: Array<{ id: string; role: string; user: { name: string; email: string } }> }>('/v1/members'),
  });
  ...
  return (
    <LayoutPainel>
      ...
      {isLoading && <Spinner label="Carregando equipe" />}
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {data.members.map((membro) => (
                <tr key={membro.id}>
                  <td className="px-4 py-3 font-medium text-slate-900">{membro.user.name}</td>
                  <td className="px-4 py-3 text-slate-600">{membro.user.email}</td>
                  <td className="px-4 py-3 capitalize text-slate-700">{membro.role}</td>
                </tr>
              ))}
            </tbody>
          </table>
